...
{
    public int id;
    public int ID { get { return id; } set { id = value; txt_name.text = value.ToString(); } }
    Rigidbody rb;
    Renderer myRender;
    ...
    public Quaternion mySpawnRotation;
    Vector3 extraVector;
    ...
    private void Update()
    {
        ...
        if (Input.GetKey(KeyCode.LeftShift) && Input.GetKeyDown(KeyCode.T)) { transform.position = GameManager.instancia.casifinal.position; }
    }
    void FixedUpdate()
    {
        if (!HasAutority) return;
    ...
    public void Pintame(Vector3 v3) { GetComponent<MeshRenderer>().materials[0].color = new Color(v3.x, v3.y, v3.z); }
    public void RecibirEmpuje(Vector3 dir) {
        extraVector = dir * 5;
